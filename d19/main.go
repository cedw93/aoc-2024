...
func possibleDesign(design string, tps []string) bool {
	...
	// '' so return true as valid design
	for _, possiblePattern := range designGraph[target[0]] {
		if strings.HasPrefix(target, possiblePattern) && possibleDesign(target[len(possiblePattern):], tps) {
			return true
		}
	}
